...
$search = Get-ADComputer -Identity $server -ErrorAction SilentlyContinue

if($search -ne $null) {Write-Host "$server All Ready Exists" -ForegroundColor Red;break}
...
'@
Invoke-VMScript -VM $server -ScriptText $VMBETAAPP01 -ScriptType Powershell -GuestUser $guestuser -GuestPassword $guestpassword -HostUser $hostuser -HostPassword $hostpassword
